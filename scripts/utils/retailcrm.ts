...
function getBaseUrl(): string {
  const raw = requireEnv("RETAILCRM_BASE_URL").replace(/\/+$/, "");
  return raw;
}
...
export async function createOrderInRetailCrm(order: Record<string, unknown>, site: string): Promise<RetailCrmCreateResponse> {
...
export async function listRetailCrmOrders(site: string, page: number, limit: number): Promise<RetailCrmListResponse> {
...
export type { RetailCrmOrder, RetailCrmListResponse, RetailCrmCreateResponse };
